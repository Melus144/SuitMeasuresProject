{{-- VIDEOS SENSE COSTILLAR --}}
@foreach($pasos_sin_costillar as $key => $paso)
    <div id="step_sin_costillar_{{$key}}"
         x-bind:class="{ 'hidden' : step != {{$key}} || costillar != 'sin_costillar' }"
         class="hide h-[calc(100vh-42px)]">
        <div class="w-full h-[calc(100vh-140px)] bg-black">
            <div class="step-indicator pt-2">
                <h4 class="text-white text-center">{{$key}}/{{count($pasos_sin_costillar)}} {{__($paso['title'])}}</h4>
            </div>
            ...
            <div class="w-full bg-gray-200 rounded-full h-2.5 mb-4 dark:bg-gray-700">
                <div class="bg-brand-red h-2.5 rounded-full"
                     style="width: {{100/count($pasos_sin_costillar)*$key}}%;"></div>
            </div>

            {{-- Reproductor de Video --}}
            <div
                class="relative container overflow-hidden h-[calc(100vh-179px)] max-w-6xl md:mx-auto flex flex-col justify-center items-center align-middle align-items-center">
                <video ref="video"
                       x-ref="sin_costillar_{{$key}}_video"
                       muted controls
        ...
        <div class="p-4">
            <div class="p-1 flex items-center justify-center">
                ...
                <div>
                    <div class="flex mx-4" x-ref="sin_costillar_{{$paso['input']}}">
                        <input name="sin_costillar[{{$paso['input']}}]"
                               x-model="dades.sin_costillar.{{$paso['input']}}"
                               @input="localStorage.setItem('dades', JSON.stringify(dades))"
                               placeholder="0"
                               x-ref="sin_costillar_{{$key}}_input"
                               type="text"
                               maxlength="5"
                        ...
                        <span class="h-14 w-32 bg-white text-md flex items-center pt-1.5">cm</span>
                        {{--Si en un futur hi ha mes unitats de mesura, enviar-ho vía array controlador --}}
                    </div>
                </div>
                <div>
                    <a class="mx-auto text-center block button-red transition-all hover:bg-brand-red-dark w-14 h-14 py-3 px-4 text-lg"
                       x-on:click="next_step('sin_costillar', '{{$paso['input']}}')"
                       href="#">
                        <svg version="1.1" xmlns="http://www.w3.org/2000/svg"
                             xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="215.4px"
                             height="22px" viewBox="0 0 215.4 459.8"
